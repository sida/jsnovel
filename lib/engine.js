...
function create(scriptObj){
    ...
    var scriptCounter = {'val':0}; //参照渡しにしたいのでオブジェクトにしている
    var varFrame = {};
    if ('start' in labelIdx){
	scriptCounter.val = labelIdx['start'];
    }
    return {
	'step': function(){
	    return step(code,labelIdx,scriptCounter,varFrame);
	}
    };
}

function step(code,labelIdx,scriptCounter,varFrame){
    if (code.length <= scriptCounter.val){return false;}
    var c = code[scriptCounter.val];
	...
	break;
    case 'g': // goto
	scriptCounter.val = labelIdx[arg];
	break;
    case 'i': // if
	var retStif = statmentIf(scriptCounter,labelIdx,varFrame,arg);
	break;
    case 'e': // end
	return false;
    default:
...
function statmentIf(scriptCounter,labelIdx,v,arg){
	...
	scriptCounter.val = labelIdx[labelOrExp];
	return true;
    }
    else{
	//TODO エラー処理
